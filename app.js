...
async function actualizarResultadoPartido(partidoId, golesLocal, golesVisita) {
    ...
    // Marcar como finalizado y guardar resultado
    ...
    console.log(`✅ Partido ${partido.local} vs ${partido.visita}: ${puntosAsignados} usuarios recibieron puntos`);
    webApp.showAlert(`Resultado actualizado: ${partido.local} ${golesLocal} - ${golesVisita} ${partido.visita}\nPuntos asignados a ${puntosAsignados} usuarios`);
    
    // Actualizar la lista de partidos en Firebase para que los usuarios vean el resultado
    await database.ref(`partidos/${partidoId}`).set({
        local: partido.local,
        ...
        return `
            <div class="partido-card ${yaPredicho ? 'predicho' : ''} ${estaFinalizado ? 'cerrado' : ''}">
                ...
                ${!estaFinalizado && !yaPredicho ? 
                    ...
                    yaPredicho ? `<div style="text-align:center; margin-top:8px;">⏳ Esperando resultado...</div>` : ''
                }
            </div>
        `;
    }).join('');
...
document.getElementById('confirmarExacto').onclick = async () => {
    ...
    // Guardar predicción SIN dar puntos (solo se guarda, los puntos llegarán después)
    predicciones[partidoActual.id] = {
        prediccion: resultadoSeleccionado,
        goles_local: golesLocal,
        goles_visita: golesVisita,
        timestamp: Date.now(),
        puntos_ganados: 0,  // Aún no tiene puntos
        acertado: false      // Aún no se sabe
    };
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // EXPONER LA FUNCIÓN PARA ACTUALIZAR RESULTADOS DESDE LA CONSOLA
    ...
    console.log("📝 Para actualizar resultados, usa: actualizarResultadoPartido(ID, golesLocal, golesVisita)");
    console.log("📝 Ejemplo: actualizarResultadoPartido(1, 2, 1)  (Catar 2-1 Suiza)");
});