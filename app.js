// ============================================
// MUNDIAL RETO - VERSIÓN CORREGIDA
// Errores de predicción solucionados
// ============================================
...
document.querySelectorAll('.opcion-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        // Mostrar input de goles SIEMPRE (incluso para empate, para que puedan poner el marcador exacto)
        document.querySelector('.goles-input').style.display = 'block';
    });
});

document.getElementById('confirmarExacto').onclick = async () => {
    // Verificar que se seleccionó un resultado
    ...
    // Validar que los goles sean válidos
    ...
    if (resultadoSeleccionado === 'empate') {
        if (golesLocal === golesVisita) {
            if (golesLocal > 0 || golesVisita > 0) {
                puntosGanados = 3; // Empate con goles (1-1, 2-2, etc.)
                esExacto = true;
            } else {
                puntosGanados = 1; // Empate 0-0
            }
        } else {
            puntosGanados = 1; // Dijo empate pero no lo es (mal, solo 1 punto por participar)
        }
    } else if (resultadoSeleccionado === 'local') {
        if (golesLocal > golesVisita) {
            puntosGanados = 3;
            esExacto = true;
        } else {
            puntosGanados = 2; // Acertó que gana local pero no los goles
        }
    } else if (resultadoSeleccionado === 'visitante') {
        if (golesVisita > golesLocal) {
            puntosGanados = 3;
            esExacto = true;
        } else {
            puntosGanados = 2; // Acertó que gana visita pero no los goles
        }
    }
    
    // Aplicar comodín doble puntos
    let multiplicador = 1;
    if (powerups.double > 0) {
        multiplicador = 2;
        powerups.double--;
        webApp.showAlert(`🔮 ¡COMODÍN ACTIVADO! Puntos duplicados`);
    }
    
    puntosGanados = puntosGanados * multiplicador;
